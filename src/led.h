...
#define updateTmpDisplay() { uint8_t tmp; \
                        tmp=ledtable[tmpbuf[0]]; if (dot0) tmp|=0x80; dbuf[3]=tmp; \
                        tmp=ledtable[tmpbuf[1]]; if (dot1) tmp|=0x80; dbuf[2]=tmp; \
                        tmp=ledtable[tmpbuf[3]]; if (dot3) tmp|=0x80; dbuf[0]=tmp; \
                        tmp=ledtable[tmpbuf[2]]; if (dot2) tmp|=0x80; dbuf[1]=tmp; }
